            ...
            <form action="{{ route('restaurants.orders.addToOrder', [$restaurant, $order]) }}" method="POST">
                ...
                <div class="mb-4 flex flex-col">
                    ...
                    <input type="text" name="name" placeholder="Item Name"
                        class="border border-gray-300 rounded-md p-2 w-full mb-4" autofocus>
                    <label>Quantity:</label>
                    <input type="number" name="quantity" placeholder="quantity" value="1"
                        class="border border-gray-300 rounded-md p-2 w-full mb-4">
                </div>
                <button type="submit"
                    class="text-white bg-orange-600 hover:bg-orange-700 px-4 py-2 rounded-md font-semibold transition-colors duration-200">Add</button>
            </form>
        </div>
    </div>
</x-public-layout>
